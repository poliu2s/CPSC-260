...
void oprintTestResult( const FullTimeEmployee &anEmployee,
		      int aYear,
		      int aPayPeriod,
			  ofstream ofs1)
// Output the results of a file.
{
    ofs1 << "\n" << aYear << " pay period #" << aPayPeriod
       << " salary = $" 
       << anEmployee.getPayment( aYear, aPayPeriod );
...
void otestEmployee( const FullTimeEmployee& anEmployee,
                   string aName,
	               int aYear,
	               int aPayPeriod,
	               float aMonthlyWage,
...
// Create an object using the test data, and print the results.
{
    ...
    for ( int i = start - 1; i <= start + 2; i++ )
    {
        ...
        // one and two pay periods after the employee has started.
        calculateTarget( anEmployee, i, targetYear, targetPayPeriod, payPeriods );
        oprintTestResult( anEmployee, targetYear, targetPayPeriod, ofs2 );
    }
...
// Library facilities used: cout and operator<< from iostream
{
	...
	//otestEmployee( theEmployee1, theName, theYear, thePayPeriod, theMonthlyWage, outFile ); 
    ...
    testEmployee( theEmployee3, theName, theYear, thePayPeriod, theMonthlyWage ); 
